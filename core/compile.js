...
class Compile extends Hook {
  ...
  start () {
    ...
    const parserType = path.extname(this.options.entry) !== this.options.wpyExt
      ? 'component'
      : 'wpy'

    this.hookUnique('wepy-parser-' + parserType, { path: this.options.entry, type: 'app' }).then(app => {
      ...
      let tasks = pages.map(v => {
        let file;
        const { scriptLang } = CONST.LANG_MAP
        ...
        file = v + Object.keys(scriptLang)
          .find(extname => {
            return fs.existsSync(v + extname)
          });
        if (
          fs.existsSync(file)
          ) {
