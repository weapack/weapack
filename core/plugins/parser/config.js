...
exports = module.exports = function () {
  this.register('wepy-parser-config', function (rst, ctx) {
    ...
    let plist = componentKeys.map(name => {
      let url = config.usingComponents[name];

      if (path.isAbsolute(url)) {
        const fixUrl = url.slice(1);
        const srcPath = path.resolve(this.context, this.options.src);
        const componentPath = path.join(srcPath, fixUrl);
        const contextDir = path.dirname(ctx.file);

        url = path.relative(contextDir, componentPath);
      }
